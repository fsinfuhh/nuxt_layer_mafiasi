...
<template>
  <UCard>
    ...
    >
      <h1 class="text-2xl">The response from Mafiasi-Identity is being processed...</h1>
    </template>

    <template #default>
      <h1 v-if="!isProcessingTimedOut" class="text-2xl">The response from Mafiasi-Identity is being processed...</h1>

      <p v-if="isProcessingTimedOut" class="mb-4">
      ...
      <UButton v-if="isProcessingTimedOut" :to="{ name: 'login' }" size="lg">Retry</UButton>
    </template>
  </UCard>
</template>
